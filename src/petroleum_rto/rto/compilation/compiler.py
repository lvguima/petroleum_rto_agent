"""Compile candidate vectors into strictly paired simulator requests."""
...
from ..ports.interfaces import ProviderRequestFactory


@dataclass(frozen=True)
class CompiledPair:
    """Baseline/candidate requests compiled from one immutable problem."""
...
class CompilationError(ValueError):
    """Base class for classified failures in candidate compilation."""


class CandidateCompilationError(CompilationError):
    """The candidate payload is incompatible with its immutable problem."""


class SystemCompilationError(CompilationError):
...
class CandidatePlanCompiler:
    ...
    def _compile_trusted_pair(
        ...
        *,
        ...
        assert_compiled_pair(pair, problem, self._catalog)
        if stage == "M4":
