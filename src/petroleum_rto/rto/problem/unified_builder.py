...
from ..capabilities import BundleCapabilityView, UnifiedCapabilityBundle
...
from ..unified_inputs import IntentResolver, OptimizationIntent


class UnifiedProblemBuilder:
    ...
    def build(
        ...
        if context.context_schema_ref != bundle.context_schema.ref:
            raise ValueError("context references another ContextSchema")
        self._validate_context(bundle, context)
        ...
        self._validate_decision_cardinality(bundle, tuple(item.variable_id for item in decisions))
    ...
    def _validate_context(
        ...
        for field in bundle.context_schema.fields:
            if not field.required:
                continue
            if field.field_id in direct:
                continue
            if field.field_id == "current_setpoints" and context.current_setpoints:
                continue
            if field.field_id == "initial_inventory_ratios" and context.initial_state:
                continue
            if field.field_id in context.facts:
                continue
            raise ValueError(f"trusted context is missing required field {field.field_id!r}")
    ...
    @staticmethod
    def _validate_decision_cardinality(
        bundle: UnifiedCapabilityBundle,
        decision_ids: tuple[str, ...],
    ) -> None:
        for rule in bundle.system_policy.compatibility_rules:
            if rule.rule_type != "cardinality" or rule.subject_kind != "decision":
                continue
            count = len(set(decision_ids) & set(rule.subject_ids))
            assert rule.minimum_count is not None and rule.maximum_count is not None
            if not rule.minimum_count <= count <= rule.maximum_count:
    ...
    @staticmethod
    def _hard_constraints(bundle: UnifiedCapabilityBundle) -> tuple[ConstraintRule, ...]:
