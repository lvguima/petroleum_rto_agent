...
class UnifiedM4PairedEvaluator:
    ...
    def __init__(self, problem: OptimizationProblem, catalog: CapabilityCatalog) -> None:
        ...
        if problem.evaluation_plan.dynamic_stage != "M4":
            raise ValueError("unified M4 evaluator requires M4 as the dynamic stage")
        ...
        if not rules:
            raise ValueError("unified M4 evaluation requires an M4 hard constraint")
    ...
    def validate_baseline(
        ...
        if pair.baseline.stage != "M4":
            raise ValueError("unified M4 baseline requires an M4 request")
        if (
    ...
    def evaluate(
        ...
        if pair.baseline.stage != "M4" or pair.candidate.stage != "M4":
            raise ValueError("unified M4 evaluator requires an M4 pair")
...
class UnifiedM4EvaluationService:
    ...
    def __init__(
        ...
        compiler: UnifiedCandidatePlanCompiler,
        ...
        self._evaluator = UnifiedM4PairedEvaluator(problem, catalog)
...
__all__ = ["UnifiedM4EvaluationService", "UnifiedM4PairedEvaluator"]
