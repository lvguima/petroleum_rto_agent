"""Unified paired M2 evaluation and cached simulator execution service."""
...
class UnifiedM2PairedEvaluator:
    ...
    def __init__(
        ...
        if problem.evaluation_plan.static_stage != "M2":
            raise ValueError("unified M2 evaluator requires M2 as the static stage")
        ...
        if not static_constraints:
            raise ValueError("unified M2 evaluation requires at least one M2 hard constraint")
    ...
    def evaluate(
        ...
        if pair.baseline.stage != "M2" or pair.candidate.stage != "M2":
            raise ValueError("unified M2 evaluator requires an M2 pair")
...
class UnifiedM2EvaluationService:
    ...
    def __init__(
        ...
        self._evaluator = UnifiedM2PairedEvaluator(problem, catalog, formula_registry)
