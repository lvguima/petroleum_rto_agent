...
def exclusive_file_lock(path: Path, *, label: str) -> Iterator[None]:
    ...
    with ExitStack() as directory_guard:
        if sys.platform == "win32":
            from petroleum_rto._windows_files import guard_directory

            directory_guard.enter_context(guard_directory(path.parent, create=True))
        ...
        try:
            ...
            if sys.platform == "win32":
                from petroleum_rto._windows_files import open_private_file

                descriptor = open_private_file(lock_path, writable=True)
            else:
