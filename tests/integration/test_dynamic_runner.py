...
def _total_inventory_kg(state: DynamicState) -> float:
    return sum(
        inventory.total_mass_kg for inventory in state.liquid_inventories.values()
    ) + sum(state.top_gas_component_masses_kg.values())
...
def test_feed_step_scenario_accumulates_inventory_without_hidden_control(
    ...
    assert result.samples[-1].state.actuator_states[
