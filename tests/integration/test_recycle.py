...
def test_default_recycle_converges_with_decreasing_residuals(
    ...
    decreases = sum(
        later <= earlier for earlier, later in pairwise(default_solve.residual_history)
    )
...
def test_zero_reflux_with_nominal_heat_recovery_exactly_reduces_to_m1(
    ...
    assert recycled.diagnostics["furnace_process_duty_w"] == m1_result.diagnostics[
        "furnace_process_duty_w"
    ]
    assert recycled.diagnostics["furnace_fuel_duty_w"] == m1_result.diagnostics[
        "furnace_fuel_duty_w"
    ]
...
def test_initial_reflux_guesses_converge_to_same_solution(
    ...
    high_estimate = oil_estimate.at_conditions(
        mass_flow_kg_s=1.8 * oil_estimate.mass_flow_kg_s
    )
    solved = [
...
def test_higher_reflux_ratio_increases_realized_ratio_and_not_gasoline_t90(
    ...
    assert higher.diagnostics["realized_reflux_ratio"] > baseline.diagnostics[
        "realized_reflux_ratio"
    ]
    assert higher.qualities["gasoline"]["t90_k_proxy"] <= baseline.qualities["gasoline"][
        "t90_k_proxy"
    ]
...
def test_more_recovered_heat_reduces_furnace_fuel_at_fixed_outlet(
    ...
    assert high.diagnostics["actual_recovered_duty_w"] > low.diagnostics[
    ...
    assert high.diagnostics["furnace_fuel_duty_w"] < low.diagnostics[
        "furnace_fuel_duty_w"
    ]
