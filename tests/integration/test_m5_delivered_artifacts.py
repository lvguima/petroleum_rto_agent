...
def test_delivered_m5_suite_matches_a_fresh_source_verified_run(
    ...
    for name, relative_path in regenerated.paths.items():
        delivered = repo_root / relative_path
        replayed = tmp_path / relative_path
    ...
    gold = _json_object(repo_root / regenerated.paths["reconciled_case"])
    parameters = _json_object(
        repo_root / regenerated.paths["calibrated_parameters"]
    )
    report = _json_object(repo_root / regenerated.paths["report_json"])
    manifest = _json_object(repo_root / regenerated.paths["artifact_manifest"])
    ...
    for directory in (repo_root / "data/gold", repo_root / "configs/parameters", repo_root / "reports/modeling"):
