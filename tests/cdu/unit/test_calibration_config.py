...
def _paths(repo_root: Path) -> tuple[Path, Path, Path]:
    return (
        ...
        repo_root / "configs/cdu/models/cdu_mini_v0.1.0.json",
        repo_root / "configs/cdu/cases/case_20260604.json",
    )
...
def test_compatibility_rejects_base_model_prior_drift(repo_root: Path) -> None:
    ...
    case = load_case_config(case_path)
    catalog = load_component_catalog(
        resolve_cdu_repository_path(repo_root, drifted_model.component_catalog_path)
    )
