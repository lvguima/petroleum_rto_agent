...
_FLASH_NAPHTHA_STATE = "liquid_inventories.flash_drum.component_masses_kg.naphtha"
...
class _ToyDynamicModel:
    ...
    def __init__(
        ...
        *,
        ...
        self._reported_salt_rate_kg_s = (
            salt_rate_kg_s if reported_salt_rate_kg_s is None else reported_salt_rate_kg_s
        )
    ...
    def evaluate(
        ...
        derivative[self._temperature_index] = (
            commands["temperature_target_k"] - state.thermal_states["furnace_outlet_temperature_k"]
        ) / 20.0
...
def real_dynamic_model(repo_root: Path) -> OpenLoopDynamicModel:
    model: ModelConfig = load_model_config(repo_root / "configs/cdu/models/cdu_mini_v0.1.0.json")
    case: CaseConfig = load_case_config(repo_root / "configs/cdu/cases/case_20260604.json")
...
def test_real_nominal_model_holds_for_four_hours_and_conserves(
    ...
    assert result.versions["scenario_version"] == (_REAL_BASELINE_METADATA["scenario_version"])
    assert (
        result.conservation_tolerances.instantaneous_mass_atol_kg_s
...
def test_step_direction_and_time_step_sensitivity() -> None:
    ...
    coarse_final = coarse.samples[-1].state.thermal_states["furnace_outlet_temperature_k"]
...
def test_non_aligned_right_continuous_step_has_no_pre_event_effect() -> None:
    ...
    assert after_event.state.actuator_states["fresh_feed_flow_kg_s"] == pytest.approx(12.0)
