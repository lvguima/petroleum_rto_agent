...
def _assert_instantaneous_conservation(evaluation: DynamicEvaluation) -> None:
    for component in ALL_COMPONENTS:
        assert evaluation.boundary_balance.component_residuals_kg_s[component] == pytest.approx(
            0.0, abs=_COMPONENT_TOLERANCE_KG_S
        )
...
def _assert_complete_product_observations(
    ...
    for product in _NET_PRODUCT_NAMES:
        ...
        assert total_flow == pytest.approx(
            evaluation.stream_mass_flows_kg_s[_PRODUCT_STREAM_FLOW_NAMES[product]],
            abs=1e-12,
        )
        ...
        observation_stream = MaterialStream(
            ...
            mass_fractions={
                component: component_flows[component] / total_flow for component in ALL_COMPONENTS
            },
        )
...
def test_nominal_product_observations_are_complete_frozen_and_serializable(
    ...
    evaluation = dynamic_case.dynamic_model.evaluate(dynamic_case.dynamic_model.initial_state)
...
def test_perturbed_product_observations_remain_complete_and_serializable(
    ...
    assert perturbed.stream_mass_flows_kg_s["kerosene"] > nominal.stream_mass_flows_kg_s["kerosene"]
...
def test_initializer_rejects_unsuccessful_and_nonconserved_recycle_results(
    ...
    bad_component_residuals = dict(valid_steady.balance.component_residuals_kg_s)
...
def test_initializer_uses_configured_dynamic_material_tolerances(
    ...
    def evaluate_with_mass_residual(
        ...
        invalid_balance = replace(
            ...
            inlet_kg_s=(evaluation.boundary_balance.inlet_kg_s + injected_residual_kg_s),
        )
        return replace(
            evaluation,
            boundary_balance=invalid_balance,
            boundary_mass_in_kg_s=(evaluation.boundary_mass_in_kg_s + injected_residual_kg_s),
        )
...
def test_unknown_negative_commands_and_invalid_states_fail_explicitly(
    ...
    raw_time_constant = dynamic_case.model_config.dynamic["actuator_time_constant_s"]
    assert isinstance(raw_time_constant, (int, float))
    assert not isinstance(raw_time_constant, bool)
    expected_derivative = (above_twice_nominal[actuator_name] - baseline[actuator_name]) / float(
        raw_time_constant
    )
    ...
    fuel_index = DynamicState.vector_names().index("actuator_states.furnace_fuel_duty_w")
...
def test_same_inputs_produce_identical_initialization_and_rhs(
    ...
    assert (
        second.evaluate(second.initial_state).as_dict()
        == first.evaluate(first.initial_state).as_dict()
    )
