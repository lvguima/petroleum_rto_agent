...
def test_flash_vapor_is_included_in_column_feed(
    ...
    for component in ALL_COMPONENTS:
        column_inlet = furnace_outlet.component_flow_kg_s(
            component
...
def test_higher_flash_temperature_does_not_reduce_vapor_fraction(
    ...
    case_data = load_json(repo_root / "configs/cdu/cases/case_20260604.json")
    ...
    assert (
        hotter.diagnostics["flash_vapor_fraction"]
        >= baseline_result.diagnostics["flash_vapor_fraction"]
    )
...
def test_higher_first_cut_point_increases_gasoline(
    ...
    model_data = load_json(repo_root / "configs/cdu/models/cdu_mini_v0.1.0.json")
