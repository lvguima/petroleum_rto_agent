...
def test_artifact_payloads_separate_origins_and_only_calibrate_two_parameters(
    ...
    assert len(_sequence(field["observations"])) == len(
        result.observation_catalog_evidence
    )
    ...
    assert reconciliation_section["origin"] == (
    ...
    assert reconciliation["result_fingerprint"] == (
        result.reconciliation.result_fingerprint
    )
    assert reconciliation["data_origin"] == (
        "mixed_field_measurements_and_M2_latent_priors"
    )
    ...
    assert parameters["alignment_config_fingerprint"] == (
        result.fingerprints["alignment_object"]
    )
    assert parameters["calibration_config_sha256"] == (
        result.fingerprints["calibration_config_file"]
    )
    assert parameters["calibration_input_fingerprint"] == (
...
def test_writer_is_repeatable_and_json_markdown_disclose_the_same_full_set(
    ...
    gold = _json_object(tmp_path / first.paths["reconciled_case"])
    parameters = _json_object(tmp_path / first.paths["calibrated_parameters"])
    report = _json_object(tmp_path / first.paths["report_json"])
    manifest = _json_object(tmp_path / first.paths["artifact_manifest"])
    markdown = (tmp_path / first.paths["report_markdown"]).read_text(encoding="utf-8")
    expected = _unfitted_observation_ids(result)
    ...
    assert _mapping(report["completion_checks"])[
