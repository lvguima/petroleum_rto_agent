...
def test_unknown_model_field_is_rejected(repo_root: Path) -> None:
    raw = load_json(repo_root / "configs/models/cdu_mini_v0.1.0.json")
    raw["unexpected"] = 1
...
def test_invalid_case_unit_is_rejected(repo_root: Path) -> None:
    raw = copy.deepcopy(load_json(repo_root / "configs/cases/case_20260604.json"))
...
def test_invalid_nested_model_range_is_rejected(repo_root: Path) -> None:
    raw = copy.deepcopy(load_json(repo_root / "configs/models/cdu_mini_v0.1.0.json"))
...
def test_scenario_event_schema_is_strict(repo_root: Path) -> None:
    raw = load_json(repo_root / "configs/scenarios/open_loop_baseline_v0.1.0.json")
    raw["events"] = [
        {
...
def test_version_mismatch_is_rejected(repo_root: Path) -> None:
    model = load_model_config(repo_root / "configs/models/cdu_mini_v0.1.0.json")
    raw = load_json(repo_root / "configs/cases/case_20260604.json")
