...
def m6_config(repo_root: Path) -> M6ValidationConfig:
    return load_m6_validation_config(
...
def plan_evidence(
    ...
    def build_plan(
        ...
        def evaluator(inputs: Mapping[str, float]) -> Mapping[str, float]:
            weighted = sum(
                (index + 1.0) * inputs[item.input_id]
                for index, item in enumerate(plan.inputs)
            )
            return {
                item.output_id: (index + 1.0) * weighted
    ...
    steady_analysis, steady_uncertainty = build_plan(
        m6_config.steady_uncertainty
    )
    dynamic_analysis, dynamic_uncertainty = build_plan(
        m6_config.dynamic_uncertainty
    )
    return (
        {
...
def successful_result(
    ...
    scenarios = tuple(
        _scenario_from_spec(m6_config, spec) for spec in m6_config.scenarios
    )
    traces = {
        rule.rule_id: _trace_for_rule(rule)
...
def test_writer_publishes_the_fixed_auditable_four_file_suite(
    ...
    evidence = _json_object(tmp_path / published.paths["validation_evidence"])
    report = _json_object(tmp_path / published.paths["report_json"])
    manifest = _json_object(tmp_path / published.paths["artifact_manifest"])
    markdown = (tmp_path / published.paths["report_markdown"]).read_text(
        encoding="utf-8"
    )
    assert evidence == successful_result.as_dict()
    assert _mapping(report["scenario_summary"])["total"] == 22
    assert set(_mapping(report["plans"])) == set(
        successful_result.required_plan_ids
    )
    assert _mapping(report["protection_traces"]) == evidence["protection_traces"]
    assert _mapping(report["controller_tracking"]) == evidence[
        "controller_tracking"
    ]
    ...
    assert set(manifest_artifacts) == set(M6_ARTIFACT_PATHS) - {
        "artifact_manifest"
    }
...
def test_repeated_publication_is_byte_identical_and_contains_no_clock_time(
    ...
    manifest = _json_object(tmp_path / second.paths["artifact_manifest"])
...
def test_failed_result_only_builds_an_in_memory_failure_payload(
    ...
    assert payload["last_valid_scenario_ids"] == list(
        failed.last_valid_scenario_ids
    )
    assert set(_mapping(payload["last_valid_evidence"])) == set(
        failed.last_valid_scenario_ids
    )
...
def test_manifest_is_the_last_staged_file_published(
    ...
    assert published_targets == [
        (tmp_path / M6_ARTIFACT_PATHS[name]).resolve()
        for name in (
            "validation_evidence",
            "report_json",
            "report_markdown",
...
def test_verifier_rejects_tampered_markdown_hash_or_wrong_expected_result(
    ...
    markdown_path = tmp_path / published.paths["report_markdown"]
