> 历史归档（2026-09-11）：下述CDU、D0、M2/M4、搜索和策略接口已退役。路径与命令描述原实现，不能作为当前运行指引。

# RTO系统综合说明

## 当前Agent入口：单变量稳态比较

Agent现通过[runtime/steady.py](../../src/petroleum_rto/rto/runtime/steady.py)准备和执行唯一T-39稳态比较，由[专用适配器](../../src/petroleum_rto/rto/adapters/hysys_steady.py)调用仿真模块。流程为受信观测→准备/展示→真实确认→保存基准副本→基准与候选各自计算及边界采集→严格重建比较→结果解释/保存/重载。当前只开放已联调离散点，不搜索失收敛边界，不新增其他MV；重复性未通过时不选择最优或声明质量合格。

下文描述保留的CDU离线CLI、旧RTO核心及其既有证据合同；它们不再是当前Agent路径，本轮没有执行这些旧实验。旧库整体移除尚未完成，不能把Agent节点替换表述成全仓M2/M4代码已删除。当前准备/比较分别使用新的steady-comparison-plan/result合同，单点消费hysys-t39-point/2.0.0；旧v1单点仅能历史读取，不可混入新配对。
...
原CDU分阶段入口先读取并保存受信工况，再在准备阶段加载能力、解析严格Intent并构造一次Problem。用户确认的是已展示版本和绑定快照；执行复用该Problem，确认时不换工况。修改业务要求或更新快照须重新准备、展示并确认。独立`run_confirmed_optimization`公共API仍面向受信调用方按其原合同一次读取并构造，不是当前终端装配路径。
...
旧Agent图曾保存这一固定方案及审批、阶段回执，当前Agent已改为上文稳态合同；数据库属于Assistant层，RTO核心仍只使用中立合同与文件证据。三种完整确认输入在本地经过公共审批节点后，固定调用`solve_prepared_optimization`和`verify_prepared_optimization`。重启只展示恢复摘要；已批准未完成任务经明确`/resume`才续算，待审批任务仍须确认，普通追问不执行计算。
...
写入临界区使用本机独占文件锁，POSIX采用flock，Windows采用字节锁并保持路径句柄。文件内容仍逐项flush/fsync后替换；Windows不调用不支持的POSIX目录fsync，因此不宣称同等的断电目录项持久性。普通进程退出后的恢复依赖完整性检查，残缺产物不得伪装成已完成；断电恢复未验收。会话、RTO及仿真实机的验证范围见STATUS。
