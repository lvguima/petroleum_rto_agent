> 历史资料（2026-09-11退役标记）：旧CDU及其离线RTO实现、配置和命令已下线。本文保留原设计、数据来源及历史验收口径；其中旧路径和命令不再定义当前能力。当前入口见[项目状态](../STATUS_REACT_REBUILD.md)。

# 垂域模型与RTO通信协议

_2026-09-11 · 当前原生工具、公共审批与阶段恢复边界；保留独立D0公共合同。_

## 当前生产Agent

`rto-chat`和`python -m petroleum_rto.domain_model`共用一个原生工具Agent。除本地命令和三种整条确认输入外，自然语言进入同一个`create_agent`图；模型收到工具Schema，返回原生函数调用，代码校验参数并回传工具结果。真实用户确认后，图通过固定节点完成M2/M4，模型不能调用或选择计算阶段。旧外层分类合同、意图/确认专用提示、四动作网关和分离Chat链已移除，没有失败回退到旧分类器的路径。
...
| `assistant/native_tools.py` | 无独立活动状态的查询、方案准备/取消及结果检查；通过ToolRuntime读取并更新图状态 | 允许模型生成工况、任意路径、算法或确认动作 |
| `assistant/react.py`及会话模块 | 唯一图状态、真实输入审批、固定M2/M4节点和严格会话恢复 | 把模型复述、摘要或仅存回执当作授权及物理证据 |
...
工具合同为3.0.0，声明从严格参数模型生成，同一参数模型在本地执行前校验。当前仅向模型开放装置身份、工况读取、问题准备、方案取消、已有结果检查和全文分页；旧manage/solve/verify工具已删除。完整工具字段及条件见[Agent架构](../architecture/02_领域智能体架构与功能设计.md)。
...
sequenceDiagram
    ...
    participant U as 用户
    participant A as Agent图与模型工具循环
    ...
    alt 修改
        ...
        A->>A: 真实输入映射公共interrupt恢复动作并校验当前版本
        A->>R: 固定节点执行完整M2搜索
        R-->>A: 静态阶段结果
        A->>A: 保存静态检查点
        A->>R: 固定节点执行全部入围候选M4复核
        R-->>A: 最终结果与证据引用
        A->>A: 保存完整结果检查点
        A-->>U: 回答及程序核验结果
    end
...
准备工具接收优化目标、允许的available变量子集、返回方案数、snapshot_ref及修改时的previous_plan_ref；程序根据已发布能力校验。工具不接受`constraints`或约束ID，系统硬约束与发布改善门禁自动保留，额外业务约束尚不支持，不可静默忽略。返回方案数不是内部搜索预算。测量值、组成、初态和运行模式来自读取并保存的受信Context。Problem在准备阶段确定性构造并绑定快照，不求解、不仿真；刷新工况须重新准备、展示并确认。

只接受整条输入`/confirm`、`确认`或`确认执行`，仅忽略首尾空白；标点、引用及附加条件不被忽略。三者都由真实输入在本地转换为同一个`Command(resume=...)`公共审批动作，不需要模型请求，也没有直达计算的旁路。图核对完整原文、用户轮次、已展示版本和当前资格；同轮准备后不能确认。待审批追问、查询和切模型保留方案；进入修改路径即撤销旧资格，修改失败也不能确认旧版本。新方案必须重新展示，再由后续用户输入确认。摘要、模型解释和工具返回文本均不能自授执行资格。当前输入规则与审批实现见[工具合同](../../src/petroleum_rto/assistant/native_tools.py)及[Agent图](../../src/petroleum_rto/assistant/react.py)。

## 结果与恢复

静态回执只代表完整M2阶段，不能写成已完成动态验证。动态入口处理完整入围列表并返回最终七字段业务结果；最终排名、单位、基准和备选验证状态从受信证据投影。模型解释失败时，程序仍展示关键结果。

当前会话由官方SQLite检查点保存原始消息、模型/思考、快照、固定方案、审批、阶段回执、摘要和分页。固定方案保存完整能力包、Intent、Context和Problem，从自身内容严格重建；外部配置变化不会替换已展示的问题。启动只读取和展示恢复摘要，零模型请求、零计算：待审批方案仍须确认，已批准未完成方案只能在展示后明确输入`/resume`续接，普通追问不续算。

已有M2/M4以及取消后保留的最近结果须从磁盘严格重载并核对会话回执，证据缺失或损坏明确拒绝，不能仅凭会话状态宣称完成。完整阶段复用不新增仿真；半个阶段中断仍按原编排恢复，不保证每个候选恰好执行一次。`/result [workflow_id]`及`inspect_optimization`只接受受控编号，指定已有运行时校验内部证据，不支持任意目录或result.json路径；最近结果来自保存的会话，不按文件修改时间猜测。`/cancel`撤销后续执行并保留已有结果，`/clear`清除全部可恢复会话和授权，保留模型选择与磁盘RTO证据。
