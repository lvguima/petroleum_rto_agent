...
_更新日期：2026-09-03 · 本文说明当前离线RTO的职责、算法、评价、结果和独立证据边界；实时状态见[项目实施状态](../STATUS.md)。_
...
Agent确认时执行：加载当前能力 → 重新解析严格Intent → 读取一次最新Context → 构造一次Problem。编排器复用这一个Problem，不在运行前后为了比较再次构造。正常工况波动通过“确认时取最新值”处理，不设置两类波动规则，也不依赖旧问题指纹失效机制。
...
`/result`只轻量读取这七个字段。它保护JSON文件边界，但不把普通结果解释升级为内部workflow审计。
...
这一区分保留两个真实边界：日常问答追求直接、简洁；持久化内部证据只在显式开发诊断时承担严格检查。
