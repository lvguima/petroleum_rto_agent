...
运行目录只复制能够影响当前preset的已校验资源闭包：M2、M3、M4和便携M6分别使用4、5、6和11项资源。严格reader以manifest声明的运行种类重建同一闭包并核对精确文件集；这减少了无关资源加载，不减少边界检查。普通M2～M4运行只消费冻结的M5覆盖结果，不导入M5校正流水或M6证据重建工具链。
...
| 输入 | 当前模型能力 | 建议RTO角色 |
| --- | --- | --- |
| 新鲜原油进料负荷 | M2输入；M4有进料SP | 当前统一RTO由`OperatingContext`固定 |
| 炉出口温度目标 | M2输入；M4炉温SP | 当前统一RTO决策变量 |
| 塔顶压力目标 | M2输入；M4压力SP | 当前统一RTO决策变量 |
