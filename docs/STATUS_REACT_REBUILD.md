...
_建立：2026-09-09；最后更新：2026-09-10（Asia/Shanghai）｜状态：CLI输入编辑已修复，领域178项通过（含9项真实PTY）；此前1033项全仓验证为历史记录；五模型Responses未全部兼容，保留现有映射｜适用于下一次对话接续实施_
...
| 项目 | 当前事实 |
| --- | --- |
| 本轮交付 | 在框架复用评估基础上形成分类重构提案、旧机制删除范围、验收要求及三项用户待决行为；仅提案，尚未实施 |
| 用户已确认 | 任意自然语言统一入口、标准工具调用、小工具集合、多模型切换；费用不是现阶段首要约束；CLI输入修复此前已授权完成；本轮明确能用LangChain的能力优先使用LangChain，并要求先分类列出修改及待判断事项 |
| 本轮执行范围 | 整理可评审的重构清单，复核当前源码、现行说明与官方文档，更新本状态；不改业务代码、不安装依赖、不调用真实模型或仿真 |
| 后续阶段 | P0–P5已实现；P6旧链、确认、协议和CDX应用预算已有实现。剩余模型组合验证及自然语言应用帮助待处理，独立RTO CLI和v4合同不变 |
...
| 真实模型验证 | 本轮五模型Responses合成探针：CDX/Flash完整工具往返及生产解析器通过；Qwen/Pro诊断往返通过但推理形状不符合现生产解析器；Kimi标准输入和工具400。Flash真实Agent配合手写合成领域数据两轮prepare通过，无计算 |
...
| 当前下一步 | 交付下述分类提案，等待用户确定本机持久化、重启续算和自动换模型的行为及实施范围；当前继续仅做方案澄清。框架优先原则已明确，新增持久化依赖和运行行为尚未实施；此前待办保留 |
...
| 本轮修改 | CLI交互TTY使用readline配合input，保持读流/空行/EOF合同；新增9项PTY和4项重定向/缺少编辑器回归；同步两份现行说明及本状态文档，无新依赖或删除 |
...
| 代码验证 | 本轮领域178项通过（1.41秒，含9项真实PTY）；全src/tests Ruff、158源文件mypy、3个改动Python文件格式及3文档35本地链接/围栏通过。此前全仓1033项为上一轮结果，不冒充本轮全仓验证；定向与模块结果不累计相加 |
...
| Git状态 | 桌面feat/react-agent跟踪origin/feat/react-agent；本轮没有提交或推送，保留此前未提交改动，最终提交标识以git log为准。main保留e234c2e，两个PNG仍未跟踪，凭据/数据/环境不纳入Git |
...
- **执行记录：** 首次GitHub访问遇到本机代理不可用，移除本次命令代理后域名解析失败；正在通过获准网络访问核对远端并完成普通推送，不强制覆盖远端。
- **下一步：** 完成提交、推送及远程提交标识核对后交付可回退基线；重构仍属于待实施提案。

### LangChain/LangGraph分类重构提案（2026-09-10，待实施）

用户要求将上一轮拟议修改分类，并列出需要其判断的事项，明确能用LangChain的就用LangChain。本轮交付以下提案，不将方案视为代码完成或新依赖安装授权。

| 类别 | 拟采用能力与修改 | 拟删除或收敛 |
| --- | --- | --- |
| 运行状态 | 保留create_agent入口，用AgentState、标准消息更新及LangGraph checkpointer统一会话、固定方案、审批和阶段状态；运行时客户端与凭据不存入状态 | 框架消息之外重复维护的活动消息列表、工具对象中的重复任务状态及跨轮同步逻辑 |
| 持久化与恢复 | 用户选择本机保存后采用官方SQLite checkpointer；持久化可重建的固定方案、摘要及分页全文或有效引用，保留现有RTO证据文件；在M2/M4真实阶段边界记录恢复点 | 自行搭建任务恢复引擎的需要；只存方案ID却依赖进程内对象的恢复路径 |
| 人工确认与执行 | 优先HumanInTheLoopMiddleware和interrupt/Command恢复；模型侧确认、求解、复核调度拟收敛为一次受审批的优化执行，内部保持M2/M4顺序与各自恢复点 | manage中的confirm/keep协调、跨轮资格暂停/恢复标志、/confirm独立计算路径；实际可删范围以新路径验收及可达性为准 |
| 实时输出 | 先用稳定streaming updates/custom输出真实工具和业务阶段事件；再补模型标准流式接口，逐段显示回答；终端继续使用已修复行编辑 | 完整handle结束后集中输出的等待路径；不新增自研事件总线 |
| 上下文压缩 | 直接集成SummarizationMiddleware公共配置与状态更新；保留完整请求预算、当前轮/完整工具组保护、受信任务投影及大结果分页 | 自管摘要覆盖水位、重复历史切批和内部属性修改；公开能力无法保持的必要规则仅作局部扩展 |
| 错误与重试 | ModelRetryMiddleware处理结构化临时网络/限流/服务错误，建议普通模型请求最多额外重试2次；保留ModelCallLimitMiddleware，另明确实际请求与摘要预算，失败明确停止 | 自研重试循环的需要；避免摘要组件内置重试与外层重试叠加或未计入预算 |
| 模型适配与可选备用 | 按BaseChatModel标准补流式chunk、usage_metadata及容量信息；历史投影/预算以实际调用模型为准；自动备用获选后才接ModelFallbackMiddleware并验证允许模型顺序 | 可被标准接口覆盖的重复适配；保留DMX实际协议和原生推理差异，不强行统一五模型Responses |
| 工具与应用说明 | 继续标准工具Schema、ToolMessage、state/runtime与少量业务middleware；把实际模型/思考设置、工具能力和方案状态统一提供给回答路径，补自然语言帮助 | 重复工具结果包装和分散维护的帮助/状态描述；参数严格校验及字段级错误保留 |

**保留与实施约束：** 业务意图、固定工况/Problem、系统硬门禁、求解算法、基准配对、完整M4验证、证据严格重载和关键结果程序展示继续由领域代码负责。RTO/CDU核心不依赖LangChain；领域进度使用中立回调接入框架。审批仍一次覆盖固定完整优化；修改需求先失效旧审批，再准备、展示和确认新方案，不使用HITL的edit直接修改后执行。确认输入沿用现行合同；待确认追问不能执行，/model保留方案，/cancel取消待确认方案并保留既有结果。HITL下的追问/修改路径需先做交互验收，不能假定内建approve/reject已自动覆盖。工具合同若收敛为新入口必须显式升级，并删除无消费者旧入口。

**三项待决行为及建议：**

1. 是否在本机保存可恢复的会话和任务：建议保存当前会话原始记录、摘要、固定方案、审批状态、阶段和分页结果引用，使用本地SQLite文件及所需官方checkpoint依赖；首轮不增加历史会话列表或跨会话长期记忆。/clear拟同时清除该会话可恢复记录、方案和授权，既有计算证据仍按原规则保留；此为新增持久化后的拟议语义。
2. 重启后是否自动继续已批准但未完成的计算：建议只恢复并展示状态，等待明确继续后从可校验阶段续算。不会自动批准原本待确认的方案；已完成阶段经校验复用，半阶段可能重算，不承诺任意代码行续跑或恰好一次执行。
3. 模型失败时是否允许自动换模型：建议首批只对当前模型有限重试，仍失败则报错并保留手动选择；默认不开启fallback。用户选择自动切换后，再确定经过真实接口验证的允许模型及顺序，切换明确显示，按实际模型处理历史/容量，不悄悄改默认选择。关闭时不预建休眠备用链。

**分批顺序与验收：** 稳定阶段输出→状态/用户选定的持久化→HITL及固定执行流程→摘要/模型标准适配与文本流式→有限重试及用户选定的fallback；每批验证后删除被替代路径。重点验收中文行编辑与流式显示互不破坏、确认前零计算、追问不执行、修改必重审、M2/M4顺序与配对、真实跨进程恢复与分页可读、临时错误有限重试、不可重试错误直接失败，以及每个精确模型的真实工具往返/摘要/流式。相关模块验证通过后完成全仓回归；真实模型验证只用必要的合成输入，不将替身探针等同实网兼容。

**本轮核对与交接：** 已复核当前源码、架构说明、工作树和LangChain官方持久化/HITL/内建middleware资料，并独立复核上述用户决策范围。仅更新本状态文档，未新增或删除实现文件、依赖或服务，未提交/推送；指纹核对确认其他19个既有修改/未跟踪文件未变化，状态文档23个本地链接、围栏及差异检查通过，记录在`/private/tmp/petroleum-rto-langchain-plan-review/review.json`。未运行新自动化代码测试、真实模型或仿真，原因是本轮只有提案；上一轮框架探针和更早领域178项分别保持原证据性质。下一步是在当前授权内交付清单、澄清用户选择，确定实施范围后按小批迁移。
